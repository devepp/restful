...
abstract class WorkOrdersBaseReport extends AbstractReport
{
	...
	{
		return Table::builder('wo_orders', $alias)
			->setPrimaryKey('id')
			->addForeignKey('vt_id')
			...
			->addStringField('schedule_locations')->build();
	}

	protected function get_events($alias = 'events')
	{
		return Table::builder('events')->alias($alias)
		...
		->addDateTimeField('deleted_at')->build();
	}

	protected function get_wo_types($alias = 'class')
	{
		return Table::builder('wo_types')->alias($alias)
		...
		->addForeignKey('logs_submission_template_id')->build();
	}

	protected function get_facilities($alias = 'facilities')
	{
		return Table::builder('facilities')->alias($alias)
		...
		->addStringField('supplies_flag')->build();
	}

	protected function get_wo_jobs($alias = 'wo_jobs')
	{
		return Table::builder('wo_jobs')->alias($alias)
		...
		->addForeignKey('vfa_category_id')->build();
	}

	protected function get_wo_accounting_notes($alias = 'wo_accounting_notes')
	{
		return Table::builder('wo_accounting_notes')->alias($alias)
		...
		->addForeignKey('approver_id')->build();
	}

	protected function get_wo_accounting_note_type($alias = 'wo_accounting_note_type')
	{
		return Table::builder('wo_accounting_note_type')->alias($alias)
	...
	{
		return Table::builder('users')->alias($alias)
		...
		->addStringField('blackboard_id')->build();
	}

	protected function get_wo_work_types($alias = 'wo_work_types')
	{
		return Table::builder('wo_work_types')->alias($alias)
	...
	{
		return Table::builder('wo_reasons')->alias($alias)
	...
	{
		return Table::builder('facility_areas')->alias($alias)

		->setPrimaryKey('id')
		->addForeignKey('facility_id')
		->addStringField('name')
		->addStringField('remote_url')
		->addStringField('details')->build();
	}

	protected function get_wo_priorities($alias = 'wo_priorities')
	{
		return Table::builder('wo_priorities')->alias($alias)
	...
	protected function get_facility_rooms($alias = 'facility_rooms')
	{
		return Table::builder('facility_rooms')
			...
			->addDateTimeField('retired_at')->build();
	}
	...
	protected function get_wo_service_group($alias = 'wo_service_group')
	{
		return Table::builder('wo_service_group')
			...
			->addForeignKey('next_group_id')->build();
	}
	...
	protected function get_wo_part_note($alias = 'wo_part_note')
	{
		return Table::builder('wo_part_note')
			...
			->addForeignKey('tax_code_id')
			->addStringField('tax_code')
			->addDateTimeField('created_on')->build();
	}
}

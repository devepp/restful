...
class QueryGroupTest extends TestCase
{
	...
	{
		return [
			[$this->getNonNestedQueryGroup(), $this->getStandardSelections(), 'SELECT `assetsAlias`.`name`, `elementsAlias`.`name`, `recommendationsAlias`.`name` FROM recommendations recommendationsAlias LEFT JOIN elements elementsAlias ON elementsAlias.recommendations_id = recommendationsAlias.id LEFT JOIN assets assetsAlias ON assetsAlias.elements_id = elementsAlias.id'],
			[$this->getNestedQueryGroup(), $this->getAggregateSelections(), 'SELECT `assetsAlias`.`name` FROM assets assetsAlias LEFT JOIN (SELECT `assetsAlias`.`name` FROM assets assetsAlias LEFT JOIN elements elementsAlias ON elementsAlias.assets_id = assetsAlias.id) assetsAlias ON assetsAlias LEFT JOIN (SELECT `assetsAlias`.`name` FROM assets assetsAlias LEFT JOIN elements elementsAlias ON elementsAlias.assets_id = assetsAlias.id LEFT JOIN recommendations recommendationsAlias ON recommendationsAlias.elements_id = elementsAlias.id) assetsAlias ON assetsAlias'],
		];
	}
